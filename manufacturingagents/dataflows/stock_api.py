...
def check_data_sources() -> Dict[str, Any]:
    ...
    return {
        'mongodb_available': service.db_manager is not None and service.db_manager.is_mongodb_available(),
        'tdx_api_available': service.tdx_provider is not None,
        'enhanced_fetcher_available': True,  # 这个通常都可用
        'fallback_mode': service.db_manager is None or not service.db_manager.is_mongodb_available(),
        'recommendation': (
            "所有数据源正常" if service.db_manager and service.db_manager.is_mongodb_available() 
